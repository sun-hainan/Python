"""
K均值聚类 (K-Means Clustering) - 中文注释版
==========================================

算法类型：
    K-Means 是无监督学习中的聚类算法。
    与分类不同，聚类不需要预先知道标签，而是让算法自动发现数据中的分组。

算法原理：
    将数据划分为 K 个簇，使得：
    1. 每个数据点属于离它最近的质心（centroid）所在的簇
    2. 每个簇的质心是所有成员数据的均值

核心概念：
    - 簇（Cluster）：一组相似的数据点
    - 质心（Centroid）：簇的中心点（所有点的均值）
    - 异质性（Heterogeneity）：所有点到各自质心距离的平方和，越小越好

算法步骤：
    1. 初始化：随机选择 K 个点作为初始质心
    2. 分配：把每个数据点分配给离它最近的质心
    3. 更新：重新计算每个簇的质心
    4. 重复步骤 2-3，直到质心不再变化（或达到最大迭代次数）

损失函数（目标）：
    J = Σ ||x_i - μ_{c_i}||²

    其中 c_i 是点 x_i 所属的簇，μ_{c_i} 是该簇的质心
