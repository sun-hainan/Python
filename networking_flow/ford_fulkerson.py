"""
Ford-Fulkerson 最大流算法 - 中文注释版
==========================================

问题定义（最大流问题）：
    给定一个网络流图（带容量边），找到从源点到汇点的最大流量。

核心概念：
    - 容量（Capacity）：每条边能通过的最大流量
    - 流量（Flow）：实际通过的流量
    - 残余网络（Residual Network）：可调整的剩余容量
    - 增广路径（Augmenting Path）：从源到汇的路径，还能增加流量

Ford-Fulkerson 算法思想：
    1. 初始化所有流量为 0
    2. 用 BFS 找一条增广路径
    3. 增加该路径的流量（取路径上最小的剩余容量）
    4. 更新残余网络
    5. 重复直到没有增广路径
