...
核心操作：
    - insert_head: 头部插入 O(1)
    - insert_tail: 尾部插入 O(n) 或 O(1)（如果维护尾指针）
    - insert_nth: 指定位置插入 O(n)
    - delete_head: 头部删除 O(1)
    - delete_tail: 尾部删除 O(n)
    - delete_nth: 指定位置删除 O(n)
    - reverse: 反转链表 O(n)
"""
