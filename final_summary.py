...
算法实现：final_summary.py / final_summary



本文件实现 final_summary 相关的算法功能。
...
excluded_files = {
    ...
    'list_dirs.py', 'test_fix_remaining.py', 'final_summary.py',
...
for f in sorted(need_cn):
    ...
    if 'euler' in dirpath.lower() or 'xe9' in dirpath.encode('utf-8').hex():

        pe_files.append(f)

    elif any(basename.startswith(x) for x in ['fix_', 'check_', 'test_', 'scan_', 'find_', 'count_']):
...
print('OK (has both main+cn):', ok_count)

print('Skipped (excluded):', skipped)

print('Need main only:', len(only_main))

print('Need cn only:', len(only_cn))

print('Need both:', len(need_both))

print()

print('--- Remaining files needing CN (' + str(len(only_cn)) + ' total) ---')

print('Project Euler files:', len(pe_files))

print('Utility scripts:', len(utility_files))

print('Category algorithm files:', len(cat_files))
