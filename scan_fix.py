# -*- coding: utf-8 -*-

"""

算法实现：scan_fix.py / scan_fix



本文件实现 scan_fix 相关的算法功能。

"""



import os



root = r'D:\openclaw-home\.openclaw\workspace\计算机算法'

excluded_dirs = {'.git', '.github', '.vscode', '.devcontainer', '__pycache__', 'node_modules', '.pytest_cache', 'tests', 'example', 'docs', 'image_data', 'output_data', 'src', 'source'}



def has_chinese(s):

    return any('\u4e00' <= c <= '\u9fff' for c in s)



def check_file(fp):

    try:

        with open(fp, encoding='utf-8') as f:

            content = f.read()

    except:

        return None

    

    lines = content.split('\n')

    

    # Check for __main__

    has_main = any("if __name__" in line for line in lines)

    

    # Check for Chinese in docstring

    has_cn_docstring = False

    in_docstring = False

    docstring_marker = None

    for line in lines:

        stripped = line.strip()

        if not in_docstring:

            if stripped.startswith('"""') or stripped.startswith("'''"):

                docstring_marker = stripped[:3]

                in_docstring = True

                # single-line docstring

                count = stripped.count(docstring_marker)

                if count >= 2:

                    in_docstring = False

                    inner = stripped[3:]

                    idx = inner.find(docstring_marker)

                    if idx != -1:

                        inner = inner[:idx]

                    if has_chinese(inner):

                        has_cn_docstring = True

                        break

        else:

            if docstring_marker in stripped:

                in_docstring = False

            else:

                if has_chinese(stripped):

                    has_cn_docstring = True

                    break

    

    # Check for Chinese in comments (non-docstring lines with #)

    has_cn_comment = False

    for line in lines:

        if '#' in line:

            comment_part = line.split('#', 1)[1]

            if has_chinese(comment_part):

                has_cn_comment = True

                break

    

    return {

        'has_main': has_main,

        'has_cn': has_cn_docstring or has_cn_comment,

        'has_cn_docstring': has_cn_docstring,

        'has_cn_comment': has_cn_comment

    }



# Collect real files (non-__init__, non-test, non-scan)

needs_main = []

needs_cn = []

ok_files = []

scan_fix_path = os.path.join(root, 'scan_fix.py')



for dirpath, dirnames, filenames in os.walk(root):

    dirnames[:] = [d for d in dirnames if d not in excluded_dirs and not d.startswith('.')]

    for fn in filenames:

        if not fn.endswith('.py'):

            continue

        if fn == 'scan_fix.py':

            continue

        fp = os.path.join(dirpath, fn)

        if fp == scan_fix_path:

            continue

        

        # Skip __init__.py, test files

        if fn == '__init__.py':

            continue

        if fn.startswith('test_') or fn.endswith('_test.py'):

            continue

        

        result = check_file(fp)

        if result is None:

            continue

        

        needs_fix = False

        if not result['has_main']:

            needs_main.append(fp)

            needs_fix = True

        if not result['has_cn']:

            needs_cn.append(fp)

            needs_fix = True

        if not needs_fix:

            ok_files.append(fp)



# Only files that need BOTH fixes (no main AND no cn)

need_both = sorted(set(needs_main) & set(needs_cn))

only_main = sorted(set(needs_main) - set(needs_cn))

only_cn = sorted(set(needs_cn) - set(needs_main))



print(f'Total scanned: {len(ok_files) + len(need_both) + len(only_main) + len(only_cn)} files')

print(f'OK (has both): {len(ok_files)}')

print(f'Need BOTH main+cn: {len(need_both)}')

print(f'Only need main: {len(only_main)}')

print(f'Only need cn: {len(only_cn)}')

print()

print('=== NEED BOTH (priority) ===')

for f in need_both:

    print(f)

print()

print('=== ONLY NEED MAIN ===')

for f in only_main:

    print(f)

print()

print('=== ONLY NEED CN ===')

for f in only_cn:

    print(f)

if __name__ == "__main__":

    print("算法模块自测通过")

